...
void Mesh::InitFromAssimpMesh(const aiMesh& assimpMesh, const ModelLoadSettings& loadSettings, MeshVertex* dstVertices, uint8* dstIndices, IndexType indexType_)
{
    ...
    if(assimpMesh.HasPositions())
    {
        ...
        {
            Float3 position = ConvertVector(assimpMesh.mVertices[i]) * loadSettings.SceneScale;
...
void Model::CreateWithAssimp(const ModelLoadSettings& settings)
{
    ...
    for(uint64 i = 0; i < numMeshes; ++i)
    {
        meshes[i].InitFromAssimpMesh(*scene->mMeshes[i], settings, &vertices[vtxOffset], &indices[idxOffset], indexType);
...
void Model::GenerateBoxTestScene(const Float3& bottomBoxDimensions, const Float3& bottomBoxPosition,
                                 const Float3& topBoxDimensions, const Float3& topBoxPosition)
{
    meshMaterials.Init(1);
    MeshMaterial& material = meshMaterials[0];
    ...
    meshes.Init(2);
    meshes[0].InitBox(topBoxDimensions, topBoxPosition, Quaternion(), 0, vertices.Data(), (uint16*)indices.Data());
    meshes[1].InitBox(bottomBoxDimensions, bottomBoxPosition, Quaternion(), 0, &vertices[NumBoxVerts], (uint16*)&indices[NumBoxIndices * sizeof(uint16)]);
