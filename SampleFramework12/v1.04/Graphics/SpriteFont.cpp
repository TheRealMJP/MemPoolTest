...
void SpriteFont::Initialize(const wchar* fontName, float fontSize, uint32 fontStyle, bool antiAliased, const wchar* fontFilePath)
{
    texture.Shutdown();
    ...
    try
    {
        // Load the font from a file (if necessary)
        Gdiplus::PrivateFontCollection fontCollection;
        if(fontFilePath != nullptr)
            GdiPlusCall(fontCollection.AddFontFile(fontFilePath));

        // Create the font
        Gdiplus::Font font(fontName, fontSize, fontStyle, UnitPixel, fontFilePath ? &fontCollection : nullptr);
...
{
    Float2 textSize = Float2(0.0f, 0.0f);
