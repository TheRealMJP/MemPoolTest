...
// by Tiago Sousa
float2 SampleConcentricDisc(float2 xy, float numSides, float polygonAmount)
{
    float phi, r;

    float a = 2.0f * xy.x - 1.0f;
    float b = 2.0f * xy.y - 1.0f;
...
// Based off the algorithm "A Low Distortion Map Between Disk and Square" by Peter Shirley and
// Kenneth Chiu.
float2 SampleConcentricDisc(in float2 xy)
{
    float phi = 0.0f;
    float r = 0.0f;
...
float3 SampleGGXMicrofacet(float roughness, float2 u1u2)
{
    float theta = atan2(roughness * sqrt(u1u2.x), sqrt(1 - u1u2.x));
    float phi = 2 * Pi * u1u2.y;
...
float3 SampleDirectionGGX(float3 v, float3 n, float roughness, float2 u1u2)
{
    float3 h = SampleGGXMicrofacet(roughness, u1u2);
    return normalize(2.0f * dot(h, v) * h - v);
}

float3 SampleGGXVisibleNormal(float3 wo, float ax, float ay, float2 u1u2)
{
    ...
    float r = sqrt(u1u2.x);
    float phi = (u1u2.y < a) ? (u1u2.y / a) * Pi : Pi + (u1u2.y - a) / (1.0f - a) * Pi;
    float p1 = r * cos(phi);
    float p2 = r * sin(phi) * ((u1u2.y < a) ? 1.0f : v.z);
...
float3 SampleDirectionHemisphere(float2 u1u2)
{
    float z = u1u2.x;
    float r = sqrt(max(0.0f, 1.0f - z * z));
    float phi = 2 * Pi * u1u2.y;
...
float3 SampleDirectionCosineHemisphere(float2 u1u2)
{
    float2 uv = SampleConcentricDisc(u1u2);
    float u = uv.x;
    float v = uv.y;
...
float3 SampleDirectionCone(float2 u1u2, float cosThetaMax)
{
    float cosTheta = (1.0f - u1u2.x) + u1u2.x * cosThetaMax;
    float sinTheta = sqrt(1.0f - cosTheta * cosTheta);
    float phi = u1u2.y * 2.0f * Pi;
    return float3(cos(phi) * sinTheta, sin(phi) * sinTheta, cosTheta);
}

// Returns a direction that samples a rectangular area light
float3 SampleDirectionRectangularLight(float2 u1u2, float3 sourcePos, float2 lightSize, float3 lightPos,
