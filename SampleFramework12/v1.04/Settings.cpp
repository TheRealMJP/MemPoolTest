...
void FloatSetting::SetMinValue(float newMinVal)
{
    minVal = newMinVal;
    val = Max(val, minVal);
}

void FloatSetting::SetMaxValue(float newMaxVal)
{
    maxVal = newMaxVal;
    val = Min(val, maxVal);
}
...
void IntSetting::SetMinValue(int32 newMinVal)
{
    minVal = newMinVal;
    val = Max(val, minVal);
}

void IntSetting::SetMaxValue(int32 newMaxVal)
{
    maxVal = newMaxVal;
    val = Min(val, maxVal);
}
...
{
    val = Min(initialVal, numValues - 1);
    oldVal = val;
...
void EnumSetting::ClampNumValues(uint32 num)
{
    ...
    Assert_(num > 0);
    numValuesClamp = num;
    if(val >= numValuesClamp)
        val = numValuesClamp - 1;
}
                              ...
                              float step_, ColorUnit units_)
{
    ...
    if(hdr)
    {
        ...
        initialIntensity = Max(Max(val.x, val.y), val.z);
...
void ColorSetting::SetValue(Float3 newVal)
{
    if(hdr)
    {
        ...
        float newIntensity = Max(Max(newVal.x, newVal.y), newVal.z);
        newVal /= newIntensity;
...
void SettingsContainer::Update(uint32 displayWidth, uint32 displayHeight, const Float4x4& viewMatrix)
{
    if(opened == false)
    {
        ...
        if(ImGui::Begin("settings_button", nullptr, flags))
        {
